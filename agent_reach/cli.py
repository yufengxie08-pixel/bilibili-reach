# -*- coding: utf-8 -*-
"""
Bilibili Reach CLI — installer, doctor, and configuration tool.

Usage:
    bilibili-reach install --env=auto
    bilibili-reach doctor
    bilibili-reach configure twitter-cookies "auth_token=xxx; ct0=yyy"
    bilibili-reach setup
"""
...
def main():
    ...
    parser.add_argument("--version", action="version", version=f"Bilibili Reach v{__version__}")
    ...
    # ── web ──
    p_web = sub.add_parser("web", help="Run the local web UI for Bilibili transcription")
    p_web.add_argument("--host", default="127.0.0.1", help="Host to bind the local web UI")
    p_web.add_argument("--port", type=int, default=8765, help="Port for the local web UI")
    ...
    if args.command == "version":
        print(f"Bilibili Reach v{__version__}")
        sys.exit(0)
...
def _cmd_web(args):
    ...
    from agent_reach.webapp import run_web_app
